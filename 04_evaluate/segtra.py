...
def relabel(tracks, track_graph):
    ...
    new_values = np.arange(1, len(labels) + 1, dtype=np.int16)
...
def evaluate_segtra(res_tracks, res_track_graph, gt_tracks, gt_track_graph):
    ...
    res_tracks = res_tracks.astype(np.int16)
    gt_tracks = gt_tracks.astype(np.int16)
